...
const favoriteIds = JSON.parse(localStorage.getItem('favoriteIds')) || [];
    // if favoriteIdsturns null then return an empty array
    ...
    if (favoriteIds.length === 0 && messageContainer) {
...
async function renderFavoriteRecipes() {
    ...
    if (recipes.length === 0) { 
        ...
        // Burada listeleme veya kart ekleme kodları olabilir
        // Fotoğrafı ekle
        const favImgDiv = `
        <div class="fav-img"> 
        ...
        messageContainer.innerHTML += favImgDiv; // Fotoğrafları message container içerisine ekliyoruz
    }
